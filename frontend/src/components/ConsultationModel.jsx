...
const ConsultationModal = ({ isOpen, onClose }) => {
  const [formData, setFormData] = useState({
    ...
    city: "",
    interest: "",
    connectionMethods: ["phone"], // can include 'phone' and/or 'whatsapp'
    additionalInfo: "",
  });
  ...
  setFormData((prev) => {
    ...
    return {
      ...prev,
      connectionMethods: exists
        ? prev.connectionMethods.filter((m) => m !== method) // unselect
        : [...prev.connectionMethods, method], // select both allowed
    };
  });
};
    ...
    <div className="fixed inset-0 z-50 flex items-stretch justify-center bg-black/75 font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <div className="relative w-full max-w-xl bg-white shadow-2xl h-[100dvh] max-h-[100dvh] overflow-y-auto text-gray-800 p-5 sm:p-6 flex flex-col">
        ...
        >
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full px-3.5 py-2.5 bg-white border border-gray-200 rounded-lg text-sm text-gray-500 outline-none focus:border-[#d49570] focus:ring-1 focus:ring-[#d49570] appearance-none cursor-pointer transition"
              >
          ...
          {/* Row 5: Preference Toggles (Phone Call / WhatsApp) */}
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-800 mb-1.5">
...
className={`flex items-center justify-center gap-2 py-2.5 px-4 rounded-md border text-sm font-medium transition ${
  formData.connectionMethods.includes("whatsapp")
    ...
    : "bg-white border-gray-200 text-gray-600 hover:border-gray-300"
          ...
          {/* Row 6: Additional Requirements (Textarea) */}
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-800 mb-1.5">
          ...
          <button
            type="submit"
            className="w-full bg-[#d49570] hover:bg-[#c98358] text-white font-medium py-3.5 px-4 rounded-md flex items-center justify-center gap-2 text-base shadow-none hover:shadow-[5px_5px_0px_0px_#000000] hover:-translate-x-[2px] hover:-translate-y-[2px] transition-all duration-200 mt-2"     >
          ...
          <div className="flex items-center justify-center gap-1.5 text-xs text-gray-500 pt-1">
            <svg
              className="w-3.5 h-3.5 text-[#d49570]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
